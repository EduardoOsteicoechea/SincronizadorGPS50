   ...
   public class UnexsistingProvidersSynchronizationWorkflow : IUnexsistingEntitySynchronizationWorkflow<GestprojectProviderModel>
   {
      ...
      )
      {
         try
         {
            ...
            RequestExistingEntitiesLinking(GestprojectConnectionManager, Sage50ConnectionManager, tableSchemaProvider, SynchronizationDialogResult, ExistingEntities);
            RequestUnexistingEntitiesCreation(GestprojectConnectionManager, Sage50ConnectionManager, tableSchemaProvider, SynchronizationDialogResult, UnexistingEntities);
         }
      ...
      void GenerateSynchronizationWorkflowDialog()
      {
         ...
         {
            dialogMessage = $"Partiendo de la selección encontramos {UnexistingEntities.Count} cliente(s) desactualizados y {UnexistingEntities.Count} inexistentes en Sage50.\n\n¿Desea vincular los clientes existentes y crear los faltantes en Sage50?";
         }
         else if(ExistingEntities.Count > 0 && UnexistingEntities.Count == 0)
         {
            dialogMessage = $"Partiendo de la selección encontramos {UnexistingEntities.Count} cliente(s) que ya existen en Sage50.\n\n¿Desea vincularlo(s)?";
         }
         else if(ExistingEntities.Count == 0 && UnexistingEntities.Count > 0)
         {
            dialogMessage = $"Partiendo de la selección encontramos {UnexistingEntities.Count} cliente(s) inexistentes en Sage50.\n\n¿Desea crearlos y sincronizar sus datos?";
         };
