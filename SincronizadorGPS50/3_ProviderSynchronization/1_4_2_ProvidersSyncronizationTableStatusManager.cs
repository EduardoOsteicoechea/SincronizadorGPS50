...
{
   internal class ProvidersSyncronizationTableStatusManager : ISynchronizationDatabaseTableManager
   {
      public bool TableExists(SqlConnection connection, ISynchronizationTableSchemaProvider tableSchema)
      {
         try
         {
            connection.Open();

            string sqlString = $"SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE \"TABLE_NAME\" = '{tableSchema.TableName}'";
