   ...
   public class IssuedBillsDataTableManager : IGridDataSourceGenerator<SynchronizableIssuedInvoiceModel, Sage50IssuedBillModel>
   {
      ...
      )
      {
         try
         {
            ...
            //TransferIssuedInvoiceDataToSage();
      ...
      public void TransferIssuedInvoiceDataToSage()
      {
         try
         {
            foreach(SynchronizableIssuedInvoiceModel entity in SynchronizableEntities)
            {
               List<SynchronizableIssuedInvoiceDetailModel> entityDetails = SynchronizableEntitiesDetails.Where(
                  detail => detail.FCE_ID == entity.FCE_ID
               ).ToList();

               GenerateSageInvoice(entity, entityDetails);
            };            
         }
         catch(System.Exception exception)
         {
      ...
      async public void GenerateSageInvoice
      (
         SynchronizableIssuedInvoiceModel entity,
         List<SynchronizableIssuedInvoiceDetailModel> entityDetails
      )
      {
         try
         {
            // there seems to be a runtime issue, therefore, I'll register the data first, and use it from the sync button
            Document = new ewDocVentaTPV();
            Document._Cabecera._Cliente = entity.PAR_SUBCTA_CONTABLE.Trim();
            Document._Cabecera._FormaPago = "01";
            Document._Numero = "4";
            Document._Numero = Document._Numero.Trim().PadRight(10, ' ');

            await Task.Delay(100);

            Document._New(entity.SageCompanyNumber.Trim(), "", "");

            foreach(SynchronizableIssuedInvoiceDetailModel entityDetail in entityDetails)
            {            
               ewDocVentaLinTPV detailManager = Document._AddLinea();
               detailManager._Cuenta = entity.FCE_SUBCTA_CONTABLE;
               detailManager._TipoIva = entity.TaxCode;
